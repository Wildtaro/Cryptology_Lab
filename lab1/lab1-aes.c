...
//请补充代码
    for(int i=0;i<4;i++)
        for(int j=0;j<4;j++){
            int high = getLeft4Bit(array[i][j]);
            int low = getRight4Bit(array[i][j]);
            // int low = array[i][j]&0xF;
            // int high = (array[i][j]>>4)&0xF;
            int num = (high<<4)|low;
            array[i][j]=getNumFromSBox(num);
...
//请补充代码
    int org_arr[4];
    memcpy(org_arr,array,sizeof(int)*4);
    for(int i=0;i<4;i++)
    array[i]=org_arr[(i+step)%4];
}
...
//请补充代码
    int org_arr[4][4];
    memcpy(org_arr,array,sizeof(int)*16);
    for(int i=0;i<4;i++)
        for(int j=0;j<4;j++){
            array[i][j]=GFMul(colM[i][0],org_arr[0][j])^GFMul(colM[i][1],org_arr[1][j])^GFMul(colM[i][2],org_arr[2][j])^GFMul(colM[i][3],org_arr[3][j]);
        }
...
//请补充代码
    ...
    for(int i=0;i<4;i++)
        for(int j=0;j<4;j++){
            array[i][j]=GFMul(deColM[i][0],org_arr[0][j])^GFMul(deColM[i][1],org_arr[1][j])^GFMul(deColM[i][2],org_arr[2][j])^GFMul(deColM[i][3],org_arr[3][j]);
        }
}
...
void addRoundKey(int array[4][4], int round)
{
  //请补充代码
    ...
    for (int col = 0; col < 4; col++) {
        int keyWord = w[round * 4 + col];

        for (int row = 0; row < 4; row++) {

            int keyByte = (keyWord >> (24 - row * 8)) & 0xFF;

            array[row][col] ^= keyByte;
        }
    }
}
...
//请补充代码
    ...
    t = mergeArrayToInt(array);
    int temp = t^Rcon[round-1];
    return temp;
}
